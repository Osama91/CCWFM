...
{
    public class TblBrandSectionMailSampleViewModel : PropertiesViewModelBase
    {
        ...
        [Required(ErrorMessageResourceType = typeof(strings), ErrorMessageResourceName = "ReqRequestForSampleStatus")]
        public int? TblRequestForSampleStatus
        {
            get { return _tblRequestForSampleStatus; }
            set { _tblRequestForSampleStatus = value; RaisePropertyChanged("TblRequestForSampleStatus"); }
        }

        private TblRequestForSampleStatu _sampleStatusPerRow;

        public TblRequestForSampleStatu RequestForSampleStatusPerRow
        {
            get { return _sampleStatusPerRow ?? (_sampleStatusPerRow = new TblRequestForSampleStatu()); }
            set { _sampleStatusPerRow = value; RaisePropertyChanged("RequestForSampleStatusPerRow"); }
        }
    ...
    public class BrandSectionMailSampleViewModel : ViewModelBase
    {
        ...
        public void GetMaindata()
        {
            ...
            Client.GetTblBrandSectionMailSampleAsync(Brand, BrandSection);
        }
            ...
            if (SelectedMainRows != null)
            {
                ...
                if (res == MessageBoxResult.OK)
                {
                    foreach (var row in SelectedMainRows)
                    {
                        if (row.Iserial != 0)
                        {
                            Client.DeleteTblBrandSectionMailSampleAsync(
                                (TblBrandSectionMailSample)new TblBrandSectionMailSample().InjectFrom(row), MainRowList.IndexOf(row));
                        }
                        else
                        {
        ...
        public void AddNewMainRow(bool checkLastRow)
        {
            ...
            if (!checkLastRow || currentRowIndex == (MainRowList.Count - 1))
            {
                ...
                var newRow = new TblBrandSectionMailSampleViewModel { TblBrand = Brand, TblLkpBrandSection = BrandSection };
        ...
        public void SaveMainRow()
        {
            if (SelectedMainRow != null)
            {
                ...
                if (isvalid)
                {
                    ...
                    var saveRow = new TblBrandSectionMailSample();
                    saveRow.InjectFrom(SelectedMainRow);
                    Client.UpdateOrInsertTblBrandSectionMailSampleAsync(saveRow, save, MainRowList.IndexOf(SelectedMainRow));
                }
            }
        }
        ...
        private SortableCollectionView<TblBrandSectionMailSampleViewModel> _mainRowList;

        public SortableCollectionView<TblBrandSectionMailSampleViewModel> MainRowList
        {
        ...
        private ObservableCollection<TblBrandSectionMailSampleViewModel> _selectedMainRows;

        public ObservableCollection<TblBrandSectionMailSampleViewModel> SelectedMainRows
        {
            get { return _selectedMainRows ?? (_selectedMainRows = new ObservableCollection<TblBrandSectionMailSampleViewModel>()); }
            set { _selectedMainRows = value; RaisePropertyChanged("SelectedMainRows"); }
        }

        private TblBrandSectionMailSampleViewModel _selectedMainRow;

        public TblBrandSectionMailSampleViewModel SelectedMainRow
        {
