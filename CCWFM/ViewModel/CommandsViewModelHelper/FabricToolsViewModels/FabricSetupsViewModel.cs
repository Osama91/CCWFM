...
{
    public class FabricSetupsViewModel : ViewModelBase
    {
        ...
        public FabricSetupsViewModel(string fabAttrCode, int fabCategoryId, bool isLoadingForDyed)
        {
            InitializeObject();
            ObjStatus = new ObjectStatus
            {
            ...
            Client.GetFabAttributesCompleted += (s, e) =>
            {
                if (e.Error == null)
                {
                    ...
                    FabricSetupsViewModelMapper.MapToViewModelObject(this, e.Result);
                    ...
                    Client.GetFabContentCompositionsCompleted += (s1, e1) =>
                      {
                          foreach (var item in e1.Result)
                          {
                              ContentCompositionList
                                  .Add(FabricSetupsViewModelMapper.MapToViewModelObject(item));
                          }
                    ...
                    Client.GetFabContentCompositionsAsync(fabAttrCode, fabCategoryId);
                }
                else
                {
                    throw e.Error;
        ...
        public void DeleteFabricAttr()
        {
            ...
            Client.DeleteFabAttributesAsync(FabricSetupsViewModelMapper.MapToModelObject(this), LoggedUserInfo.Iserial);
        }
        ...
        public void InsertFabricAttr()
        {
            ...
            if (ObjStatus.IsReadyForSaving)
            {
                ...
                Client.AddFabAttributesCompleted += (s, e) =>
                {
                    ...
                    {
                        FabricID = e.Result;
                        var temp = new ObservableCollection<_Proxy.FabricContentsComposition>();
                        foreach (var item in ContentCompositionList)
                        {
                            temp.Add
                                (FabricSetupsViewModelMapper.MapToModelObject
                                (item, FabricCategoryID, FabricID));
                        }
                        Client.AddFabCompositionsCompleted += (s2, e2) =>
                        {
                        ...
                        Client.AddFabCompositionsAsync(temp);
                    }
                    else if (e.Error.Message.Contains("FS-SIEX-2"))
                    {
                        ...
                        if (res == MessageBoxResult.OK)
                        {
                            ...
                            Client.AddFabAttributesAsync(FabricSetupsViewModelMapper.MapToModelObject(this), FabricCategoryID, true);
                        }
                        else
                        {
        ...
        public void UpdateFabricAttr()
        {
            ...
            if (ObjStatus.IsReadyForSaving)
            {
                Client.UpdateFabAttributesCompleted += (s, e) =>
                {
                    if (e.Error == null)
                    {
                        ...
                        if (!inventTransExists)
                        {
                            ...
                            Client.AddFabCompositionsCompleted += (s2, e2) => MessageBox.Show("Item successfully updated!");
                            Client.AddFabCompositionsAsync(temp);
                        }
                        else
                        {
                ...
                Client.UpdateFabAttributesAsync(FabricSetupsViewModelMapper.MapToModelObject(this), LoggedUserInfo.Iserial);
            }
            else
            {
                MessageBox.Show("Data is not valid for saving, please revise!", "Fabric Generation Tool", MessageBoxButton.OK);
