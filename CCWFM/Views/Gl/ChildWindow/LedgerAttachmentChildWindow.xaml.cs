...
{
    public partial class LedgerAttachmentChildWindow
    {
        private readonly LedgerAttachmentViewModel _viewModel;

        public LedgerAttachmentChildWindow(TblLedgerHeaderViewModel Ledger)
        {
            InitializeComponent();

            _viewModel = new LedgerAttachmentViewModel(Ledger);
        ...
        private void btnAddGalaryAttachment_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dlg.ShowDialog().GetValueOrDefault(false))
            {
                foreach (var attachment in dlg.Files)
                {
                    ...
                    _viewModel.MainRowList.Add(new GlLedgerAttachmentViewModel
                    {
                        //AttachmentPathThumbnail = buffer,
                        AttachmentPerRow = attachment,
                        TblLedgerHeader = _viewModel.HeaderRow.Iserial,
        ...
        private void BtnDownloadButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var btn = sender as Button;
            var LedgerAttachment = btn.DataContext as GlLedgerAttachmentViewModel;

            var wClient = new WebClient();
            var uri = Application.Current.Host.Source.AbsoluteUri;
            var index = uri.IndexOf("/ClientBin");
            uri = uri.Substring(0, index);
            uri = uri + "/" + LedgerAttachment.AttachmentPath;
