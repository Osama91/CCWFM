            ...
            Dictionary<string, object> valuesObjects, out int fullCount)
        {
            using (var context = new WorkFlowManagerDBEntities())
            {
                ...
                if (filter != null)
                {
                    ...
                    query = context.TblBrandBudgetDetails.Include("TblFamily1").Include("TblSalesOrderColorTheme1").Include("TblFactoryGroup1").Where(filter, parameterCollection.ToArray()).OrderBy(sort).Skip(skip).Take(take);
                }
                else
                {
                    fullCount = context.TblBrandBudgetDetails.Count(x => x.Iserial == glSerial);
                    query = context.TblBrandBudgetDetails.Include("TblFamily1").Include("TblSalesOrderColorTheme1").Include("TblFactoryGroup1").Where(x => x.TblBrandBudgetHeader == glSerial).OrderBy(sort).Skip(skip).Take(take);
                }
